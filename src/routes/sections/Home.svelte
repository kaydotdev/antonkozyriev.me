<script>
	...
	let SnowParticlesComponent;

	onMount(async () => {
		const module = await import('svelte-particles');

		SnowParticlesComponent = module.default;
	...
	let onParticlesLoaded = (event) => {
		const _particlesContainer = event.detail.particles;
	};
...
<section id="home" class="w-screen h-screen relative grid">
	...
	<div class="particles w-full h-full top-0 left-0 opacity-60 bg-black absolute z-[1]">
		<svelte:component
			this={SnowParticlesComponent}
			id="tsparticles"
